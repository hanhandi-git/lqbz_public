...
public:
    ...
    Task(TaskId id) : id_(id) {}
    
    void setData(folly::dynamic data) { data_ = std::move(data); }
    const folly::dynamic& getData() const { return data_; }
    
    TaskId getId() const { return id_; }
    
    folly::Promise<folly::dynamic>& getPromise() { return promise_; }
    folly::Future<folly::dynamic> getFuture() { return promise_.getFuture(); }

private:
    TaskId id_;
    folly::dynamic data_;
    folly::Promise<folly::dynamic> promise_;
};
